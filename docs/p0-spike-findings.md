...
## Full round-trip verified (P0 closed)

2026-07-17, after API-key auth against the internal gateway (`model_providers.OpenAI.base_url = https://api-admin.opencoco.co`, `wire_api = "responses"`, model `gpt-5.5`): `turn/start` with "Reply with exactly: PONG" → `item/started` (reasoning) → `item/started` (agentMessage, `phase: "final_answer"`) → `item/agentMessage/delta` ×2 (`"P"`, `"ONG"`) → `item/completed` with `item.text: "PONG"` → `thread/tokenUsage/updated` → `thread/status/changed: idle` → `turn/completed` (2.3s). Injection → visible context → model reply → outbound capture: the whole Layer-2 loop is protocol-confirmed.

Two additional facts from the authenticated run:
- `turn/steer` with **no active turn at all** fails `-32600 "no active turn to steer"` — no recoverable id in the message, so the client's fallback-to-`turn/start` path is the correct (and only) recovery.
- `thread/tokenUsage/updated` carries per-turn and total token usage + context-window size — free observability for P2.
